...
export async function POST(req:NextRequest){
  ...
  if(userId){
    ...
    let error:any=null;
    if(existing.data?.id){
      const res=await supabaseAdmin.from("profils").update({id:userId,email,nom:nom||null,prenom:prenom||null,role_id,actif:true}).eq("email",email);
      error=res.error;
    }else{
      const res=await supabaseAdmin.from("profils").upsert({id:userId,email,nom:nom||null,prenom:prenom||null,role_id,actif:true},{onConflict:"id"});
      error=res.error;
    }
